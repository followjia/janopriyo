...
import { Search, ShoppingBag, User, Heart, Menu, X, LogOut, LayoutDashboard, Settings } from 'lucide-react';
        ...
        <div className="bg-white/40 dark:bg-black/40 backdrop-blur-3xl rounded-[3rem] border border-white/20 px-10 py-5 flex items-center justify-between shadow-2xl shadow-black/10">
          ...
          <Link href="/" className="group flex items-center gap-4">
             <div className="h-12 w-12 bg-primary rounded-[1.5rem_0.5rem_1.5rem_0.5rem] rotate-45 group-hover:rotate-0 transition-all duration-700 flex items-center justify-center">
                <span className="text-white font-black text-2xl -rotate-45 group-hover:rotate-0 transition-all">J</span>
             </div>
             <span className="text-2xl font-black tracking-tighter hidden md:block">Janopriyo.</span>
          </Link>
          ...
          <div className="flex items-center gap-6">
            ...
            <Link 
               href="/cart" 
               className="relative group h-14 w-14 rounded-[2rem] bg-primary text-white flex items-center justify-center shadow-xl shadow-primary/30 hover:scale-110 transition-transform"
            ...
            {session ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="h-12 w-12 rounded-[1.5rem] border-2 border-primary/20 overflow-hidden hover:scale-110 transition-transform shadow-lg">
                ...
                <DropdownMenuContent align="end" className="w-64 rounded-[2.5rem] p-4 bg-white/90 backdrop-blur-3xl border-none shadow-2xl">
                   ...
                   {isAdmin && (
                     <DropdownMenuItem onClick={() => router.push('/admin')} className="rounded-2xl cursor-pointer py-3">
                       <LayoutDashboard className="mr-3 h-5 w-5 text-primary" /> Atelier Console
                     </DropdownMenuItem>
                   )}
                   <DropdownMenuItem onClick={() => router.push('/account')} className="rounded-2xl cursor-pointer py-3">
